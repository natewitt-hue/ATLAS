...
Slash commands:
  /statshub                — Open the ATLAS Oracle Stats Hub
...
# ─────────────────────────────────────────────────────────────────────────────
#  HUB VIEW — 12-button navigation
#  timeout=None + custom_id on every button = survives bot restarts
# ─────────────────────────────────────────────────────────────────────────────

class HubView(discord.ui.View):
    """
    Persistent 12-button navigation view.
    ...
    @discord.ui.button(
        label="🆚 Head-to-Head", style=discord.ButtonStyle.secondary,
        row=1, custom_id="hub:h2h",
    )
...
class OracleHubCog(commands.Cog):
    """ATLAS Oracle — stats hub shortcut command."""
    ...
    @app_commands.command(
        name="statshub",
        description="Open the ATLAS Oracle Stats Hub — analytics, standings, and intelligence.",
    )
    async def statshub(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title="🔬 ATLAS Oracle — Stats Hub",
            description=(
                "The TSL intelligence engine. Stats, trends, profiles, and AI-powered analysis."
            ),
        ...
        embed.add_field(
            name="📊 League Stats",
            value=(
                "`/stats hub` — Full stats navigation panel\n"
                "`/stats hotcold` — Hot/Cold report (league-wide or by player)\n"
                "`/stats clutch` — Clutch rankings\n"
                "`/stats power` — Live power rankings\n"
                "`/stats recap` — Weekly game recap"
            ),
            inline=False,
        )
        embed.add_field(
        ...
        embed.set_footer(text="ATLAS™ Oracle Module · TSL Stats Intelligence")
...
async def setup(bot: commands.Bot):
    ...
    await bot.add_cog(OracleHubCog(bot))
