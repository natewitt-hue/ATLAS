...
# ── Card render size ──────────────────────────────────────────────────────────
# Sized so 5 cards fit side-by-side comfortably on TABLE_W=800
CARD_W   = 120
CARD_H   = int(CARD_W * 1024 / 741)   # maintain back.png aspect ratio → 165px
CARD_GAP = 14
...
_SCORE_GAP    = 6
_CARDS_GAP    = 10
_MID_GAP      = 70    # space between dealer cards bottom and player label
_TOP_PAD      = 42
_BOT_PAD      = 38
...
def render_blackjack_table(
    ...
    lf  = _fnt(18, bold=True)
    sf  = _fnt(15)
    stf = _fnt(30, bold=True)
    inf = _fnt(14)
