...
async def play_slots(interaction: discord.Interaction, wager: int) -> None:
    ...
    embed3.set_footer(text=f"New Balance: ${result['new_balance']:,} Bucks")
...
def _build_scratch_embed(
    ...
    if revealed < 3:
        ...
        desc  = f"🏆 **TRIPLE MATCH!** All tiles: **{tiles[0]:,}** × 3 = **+{total:,} Bucks!**"
    else:
        total = sum(tiles)
        color = discord.Color.green()
        desc  = f"✅ You won **{total:,} Bucks!**"
...
async def daily_scratch(interaction: discord.Interaction) -> None:
    ...
    embed.set_image(url="attachment://scratch.png")
    embed.set_footer(text=f"Potential win: {total:,} Bucks credited to your account")
