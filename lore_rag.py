...
# Global State for Memory Efficiency
_model = None
_index = None
_metadata = None
...
def _ensure_loaded():
    """Ensures the model and DB are loaded into memory once."""
    global _model, _index, _metadata
    if _model is None:
        _model = _load_model()
    if _index is None and os.path.exists(INDEX_FILE):
        _index = faiss.read_index(INDEX_FILE)
    if _metadata is None and os.path.exists(META_FILE):
...
def build_lore_context(query: str, k=3) -> str:
    ...
    Returns an empty string if no DB is found so it doesn't pollute the AI context.
    """
    _ensure_loaded()
    
    if _index is None or not _metadata:
        return ""  # Silently fail so Gemini can fall back to Search
    ...
    try:
        ...
        for idx in indices[0]:
            if idx != -1 and idx < len(_metadata):
                # Using the dict structure generated by your ingest function
                results.append(_metadata[idx]["formatted_text"])
...
def test(query):
    """Test a query against the lore index and print the top results."""
    print(f"Testing Query: '{query}'")
    res = build_lore_context(query, k=5)
    print(res)


def add_single_message(author, content):
    """Adds a single new message to the FAISS index and metadata (live ingestion)."""
...
def init():
    ...
    try:
        _ensure_loaded()
        print("[LoreRAG] FAISS Index loaded successfully.")
        return True
    except Exception as e:
...
async def build_lore_context_async(query: str, k: int = 3) -> str:
    """
    Async wrapper around build_lore_context.
    Runs the CPU-heavy SentenceTransformer.encode() + FAISS search
    in a thread executor so it doesn't block the Discord event loop.
    """
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, build_lore_context, query, k)
